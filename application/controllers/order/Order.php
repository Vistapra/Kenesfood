    ...
    try {
        ...
        // Debug logging
        log_message('debug', 'List Parameters: ' . json_encode([
            'outletId' => $outletId,
            'tableId' => $tableId,
            'brand' => $brandType,
            'category' => $category,
        ...
        // Check and extend active session if exists
        ...
        if (!$sessionData) {
            log_message('debug', 'Session not found in list()');
            return $this->output
                ->set_content_type('application/json')
        ...
        $expireTime = new DateTime($sessionData["expire_at"]);

        if ($currentTime > $expireTime) {
            log_message('debug', 'Session expired in list(). Expire time: ' . $sessionData["expire_at"]);
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(401)
                ->set_output(json_encode([
        ...
        // Extend session
        $newExpireTime = (new DateTime())->add(new DateInterval('PT15M'));
        $this->M_Order->qBUpdate(
            $sessionData["id"],
            [
                "expire_at" => $newExpireTime->format('Y-m-d H:i:s'),
                "updated_at" => $currentTime->format('Y-m-d H:i:s')
            ]
        );
        ...
        log_message('debug', 'Products query result: ' . json_encode($products));
        log_message('debug', 'Grouped products: ' . json_encode($groupedProducts));
        ...
        if ($sessionData) {
            $this->tsmarty->assign("session", $sessionData ?: null);
        ...
        log_message('error', 'Error in list method: ' . $e->getMessage());
        if ($is_ajax) {
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(500)
		...
		public function countCart()
		{
			try {
				...
				// Calculate cart data if not in cache
		...
		private function sendSuccessResponse($code, $message, $data)
		{
			return $this->output
				->set_status_header(200)
				->set_output(json_encode([
