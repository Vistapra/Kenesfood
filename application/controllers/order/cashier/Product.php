...
class Product extends ApplicationBase
{
	...
	{
		try {
			// Set header JSON secara eksplisit
			...
			$missingParams = [];
			if (!$productId) $missingParams[] = 'productId';
			if (!$outletId) $missingParams[] = 'outletId';
			if (!$brand) $missingParams[] = 'brand';
			if (!$tableId) $missingParams[] = 'tableId';
			...
			// Ambil detail produk
			$product = $this->MOC_Product->detail($productId);
			...
			$normalizedProduct = $this->normalizeProductData($product);
	...
	// Metode baru untuk normalisasi data produk
	private function normalizeProductData($product)
	{
		// Pastikan setiap field memiliki nilai default
		return [
			...
			'package_details' => $product['is_package'] === '1'
				? ($product['package_details'] ?? null)
				: null
		];
	}
}
