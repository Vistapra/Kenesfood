...
class M_products extends CI_Model
{
	...
	{
		$this->db->select('p.*, c.cat_name, c.cat_id, ROUND(p.product_price/1000, 1) AS price_display');
		...
		$this->db->order_by('c.cat_id ASC, p.product_name ASC');
	...
	public function get_products_by_categories()
	{
		$sql = "SELECT 
		  ...
		  ROUND(p.product_price/1000, 1) AS price_display
		FROM data_categories c
		LEFT JOIN data_product p ON c.cat_id = p.cat_id
		WHERE p.product_parent = '0' 
		  AND p.product_st = '0'
		  AND c.cat_st = '0'
		ORDER BY c.cat_id ASC, p.product_name ASC";
	...
	public function catalogue_outlet_filter_category()
	{
		...
		SELECT
			ROUND(product_price/1000, 1) AS price_catalogue, 
			p.product_name,
			p.product_pict,
			p.product_desc,
			ct.cat_name,
			...
			p.stock,
			CASE 
				WHEN LENGTH(p.product_name) > 20 
				THEN CONCAT(LEFT(p.product_name, 17), '...')
				ELSE p.product_name 
			END AS product_name
		FROM data_product p
		JOIN CategoryTree ct ON p.cat_id = ct.cat_id
		WHERE p.product_parent = 0
			...
			AND p.product_brand = ?
			AND p.product_st = '0'
		ORDER BY ct.cat_id ASC, p.product_name ASC";
	}
	...
	public function get_products_by_outlet($outlet_id, $brand_type)
	{
		$sql = $this->db->select('
			...
			p.product_price,
			p.product_pict,
			p.stock,  // Explicitly select stock
			p.product_desc,
			p.cat_id,
			c.cat_name,
			ROUND(p.product_price/1000, 1) AS price_catalogue
		')
			->from('data_product p')
			->join('data_categories c', 'p.cat_id = c.cat_id', 'left')
			->where([
				'p.product_brand' => $brand_type,
				'p.product_st' => '0',
				'p.product_parent' => 0,
				'p.ek_outlet' => '0'
			])
		...
		if ($sql->num_rows() > 0) {
			$products = $sql->result_array();
			foreach ($products as &$product) {
				$product['stock'] = (int)$product['stock'];
				$product['current_stock'] = (int)$product['stock'];
			}
			return $products;
		}
		return array();
	...
	public function getProductsByBrandAndType($params)
	{
		$brand = $params['brand'] ?? null;
		$types = $params['types'] ?? ['Makanan', 'Minuman', 'Bakery'];

		$this->db->select('product_id, product_name, product_brand, product_type');
		$this->db->from('data_product');
		$this->db->where('product_parent', 0);
		$this->db->where('product_st', '0');
		...
		if (!empty($types)) {
			$this->db->where_in('product_type', $types);
		}
		...
		foreach ($results as $product) {
			switch ($product['product_brand']) {
				case 'bakery':
					$grouped_products['bakery_products'][] = $product;
					break;
				case 'kopitiam':
					$grouped_products['kopitiam_products'][] = $product;
					break;
				case 'resto':
					$grouped_products['resto_products'][] = $product;
					break;
			}
		}
